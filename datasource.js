...
const postStroj = async (stroj) => {
  await pool.query("INSERT INTO stroj (naziv_stroja) VALUES ($1);", [
    alat.nazivStroj,
  ]);
};
...
const getProjekti = async () => {
  ...
  for (let i = 0; i < projekti.length; i++) {
    projekti[i].dijelovi = (
      await pool.query(
        "SELECT sifra_dijela, naziv_dijela, kolicina_dijelova FROM dio NATURAL JOIN dijelovi_projekt NATURAL JOIN projekt WHERE sifra_projekta=$1;",
        [projekti[i].sifra_projekta]
      )
    ).rows;
...
const getProjekt = async (id) => {
  ...
  projekt.alati = (
    await pool.query(
      "SELECT sifra_alata, naziv_alata, kolicina_alata, kolicina_na_lageru FROM alat NATURAL JOIN alat_koristi NATURAL JOIN projekt WHERE sifra_projekta=$1;",
      [id]
    )
  ).rows;
  ...
  projekt.strojevi = (
    await pool.query(
      "SELECT sifra_stroja, naziv_stroja, datum_koristenja FROM stroj NATURAL JOIN koristenje_stroja NATURAL JOIN projekt WHERE sifra_projekta=$1;",
      [id]
    )
  ).rows;
...
const deleteProjekt = async (id) => {
  ...
  await pool.query("DELETE FROM projekt WHERE sifra_projekta = $1;", [id]);
};
...
const pridijeliStrojProjektu = async (koristenje) => {
  await pool.query(
    "INSERT INTO koristenje_stroja (sifra_stroja, sifra_projekta, datum_koristenja) VALUES ($1, $2, $3);",
    [
      koristenje.sifraStroja,
      koristenje.sifraProjekta,
      koristenje.datumKoristenja,
    ]
  );
};
...
const pridijeliAlatProjektu = async (koristenje) => {
  const results = await pool.query(
    "SELECT * FROM alat_koristi WHERE sifra_alata = $1 AND sifra_projekta = $2;",
    [koristenje.sifraAlata, koristenje.sifraProjekta]
  );
  if (results.rows.length > 0) {
    const kolicina = results.rows[0].kolicina_alata;
    await pool.query(
      "UPDATE alat_koristi SET kolicina_alata = $1 WHERE sifra_alata = $2 AND sifra_projekta = $3;",
      [
        kolicina + koristenje.kolicinaAlata,
        koristenje.sifraAlata,
        koristenje.sifraProjekta,
      ]
    );
  } else {
    await pool.query(
      "INSERT INTO alat_koristi (sifra_alata, sifra_projekta, kolicina_alata) VALUES ($1, $2, $3);",
      [
        koristenje.sifraAlata,
        koristenje.sifraProjekta,
        koristenje.kolicinaAlata,
      ]
    );
  }
  ...
  await pool.query(
    "UPDATE alat SET kolicina_na_lageru = $1 WHERE sifra_alata = $2;",
    [alat.kolicina_na_lageru - koristenje.kolicinaAlata, koristenje.sifraAlata]
  );
};

const pridijeliDioProjektuEdit = async (koristenje) => {
  let kolicinaZaAzurirati = 0;
  const results = await pool.query(
  ...
  if (results.rows.length > 0) {
    kolicinaZaAzurirati =
      results.rows[0].kolicina_dijelova - koristenje.kolicinaDijelova;
    await pool.query(
      "UPDATE dijelovi_projekt SET kolicina_dijelova = $1 WHERE sifra_dijela = $2 AND sifra_projekta = $3;",
      [
        Number(koristenje.kolicinaDijelova),
    ...
    await pool.query(
      "INSERT INTO dijelovi_projekt (sifra_dijela, sifra_projekta, kolicina_dijelova) VALUES ($1, $2, $3);",
      [
        koristenje.sifraDijela,
        koristenje.sifraProjekta,
  ...
  await pool.query(
    ...
    [
      Number(dio.kolicina_na_lageru) + Number(kolicinaZaAzurirati),
      koristenje.sifraDijela,
    ]
  );
};
...
const pridijeliDioProjektu = async (koristenje) => {
  ...
  if (results.rows.length > 0) {
    ...
    await pool.query(
      "UPDATE dijelovi_projekt SET kolicina_dijelova = $1 WHERE sifra_dijela = $2 AND sifra_projekta = $3;",
      [
        Number(kolicina) + Number(koristenje.kolicinaDijelova),
        koristenje.sifraDijela,
        koristenje.sifraProjekta,
      ]
    );
  ...
  await pool.query(
    "UPDATE dio SET kolicina_na_lageru = $1 WHERE sifra_dijela = $2;",
    [
      dio.kolicina_na_lageru - koristenje.kolicinaDijelova,
      koristenje.sifraDijela,
    ]
  );
};

const ukloniDioIzProjekta = async (koristenje) => {
  try {
    const results = await pool.query(
      "SELECT * FROM dijelovi_projekt WHERE sifra_dijela = $1 AND sifra_projekta = $2;",
      [koristenje.sifraDijela, koristenje.sifraProjekta]
    ...
    await pool.query(
      ...
      [
        Number(dio.kolicina_na_lageru) +
          Number(results.rows[0].kolicina_dijelova),
        koristenje.sifraDijela,
      ]
    );
    await pool.query(
